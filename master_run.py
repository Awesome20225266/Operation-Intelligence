...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="One-command pipeline: Excel -> DuckDB -> S3")
    ...
    print("=== Step 1/2: Updating local DuckDB from Excel ===")
    ...
    if rc != 0:
        print(f"Loader failed with exit code {rc}. Aborting S3 upload.")
        return int(rc)

    # 2) Upload DuckDB -> S3 (overwrites object at bucket/key)
    print("=== Step 2/2: Uploading DuckDB to AWS S3 ===")
